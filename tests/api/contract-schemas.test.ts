...
const routeSchemaMap: RouteEntry[] = [
  ...
  },
  {
    ...
    makeRequest: () =>
      createMockRequest('http://localhost:3000/api/commitments?ownerAddress=GABC123'),
  },
  {
    ...
    setupMocks: () => {
      vi.mocked(getCommitmentFromChain).mockResolvedValue(makeChainCommitmentDetail());
    },
    makeRequest: () => createMockRequest('http://localhost:3000/api/commitments/cm_detail_1'),
  ...
  },
  {
    ...
    makeRequest: () =>
      createMockRequest('http://localhost:3000/api/commitments/search?ownerAddress=GABC123'),
  },
  {
    ...
    makeRequest: () => createMockRequest('http://localhost:3000/api/marketplace/listings'),
  },
];
...
describe('API contract schemas — handler response validation', () => {
  ...
  describe.each(routeSchemaMap)('$name', (entry) => {
    const { name, handler, schema, setupMocks, makeRequest, context } = entry;

    it('returns a response that matches the declared apiContracts schema', async () => {
      expect.assertions(4);
      setupMocks();
      const req = makeRequest();
      const res = await handler(req as any, context ?? COMMON_CONTEXT, 'test-correlation-id');
  ...
  describe('empty collections', () => {
    it('GET /api/commitments — empty items array passes schema', async () => {
      expect.assertions(3);
      vi.mocked(getUserCommitmentsFromChain).mockResolvedValue([]);
      const req = createMockRequest('http://localhost:3000/api/commitments?ownerAddress=GABC123');
      const res = await CommitmentsListGET(req, COMMON_CONTEXT, 'test-correlation-id');
      const body = await res.json();
    ...
    it('GET /api/commitments/search — empty data array passes schema', async () => {
      ...
      const res = await CommitmentSearchGET(req, COMMON_CONTEXT, 'test-correlation-id');
      const body = await res.json();
    ...
    it('GET /api/marketplace/listings — empty listings array passes schema', async () => {
      ...
      const res = await MarketplaceListingsGET(req, COMMON_CONTEXT, 'test-correlation-id');
      const body = await res.json();
  ...
  describe('optional and nullable fields', () => {
    ...
    it('GET /api/commitments/[id] — nullable maxLossPercent and absent optional fields pass schema', async () => {
      ...
      const req = createMockRequest('http://localhost:3000/api/commitments/cm_opt');
